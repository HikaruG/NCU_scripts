...
- the following experience has 3 different difficulties which the tester will be asked to choose from:
    1st level: only the text value changes; choose the correct one
    2nd level: the text's font may vary;
    3rd level: Box's color, text's font may vary;
...
2. the font color has to be different from the rectangle color, or else the stimulus will not be shown correctly
"""
...
subj_difficulty = int(p_gui.data[2]) #this value needs to be an int
samples = int(p_gui.data[3])

if not(os.path.exists(subj_id)): #create a directory for each participant
    os.mkdir(subj_id)
data_name = subj_id +"_" + subj_date + "_lvl_" + str(subj_difficulty) + ".csv" # take the name, the date and the difficulty for the file name
data_path = os.path.join(subj_id,data_name)
...
text_instructions = visual.TextStim(win,
                                    text='Please click on the same word as the instructions \n Press enter to start',
                                    pos=(0,0.7)
                                    )
...
if subj_difficulty == 1:
    ...
    random_lvalue_rect = 1
    random_rvalue_rect = 1
...
for i in range(samples):
    ...
    waiting_box.text = 'Please click on: %s' %(list_colors[correct_value][0])
    waiting_box.draw()
    win.flip()
...
#creating the dataframe and saving it
data = pd.DataFrame({'id': pid,'the resp': side,'Correct': correctness, 'resp time': timings})
if os.path.exists(data_path):
    data.to_csv(data_path, mode= 'ab',header=False)#if the csv already exists, the result will be appended at the end of the file
else:
    data.to_csv(data_path)#if the file doesn't exist, it will create it with the header
win.close()